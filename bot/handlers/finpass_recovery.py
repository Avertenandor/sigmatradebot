...
async def start_finpass_recovery(
    ...
    if pending:
        await callback.message.edit_text(
            ...
            f"Создан: "
            f"{pending.created_at.strftime('%d.%m.%Y %H:%M')}\n\n"
    ...
    if has_active:
        await callback.message.edit_text(
            "✅ **Ваш запрос одобрен!**\n\n"
            "Новый финансовый пароль был отправлен вам в личные "
            "сообщения.\n\n"
            "⚠️ **Важно:**\n"
            "• Ваши выплаты заблокированы до первого использования "
            "нового пароля\n"
            "• После первого успешного вывода блокировка "
            "будет снята автоматически\n\n"
            "Используйте раздел 'Вывод' для проверки "
            "нового пароля.",
            reply_markup=InlineKeyboardBuilder()
            .row(
                InlineKeyboardButton(
