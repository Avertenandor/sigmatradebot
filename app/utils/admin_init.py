...
async def ensure_default_super_admin(
    ...
    if bot:
        try:
            ...
            logger.debug(
                f"Fetched username for admin {DEFAULT_SUPER_ADMIN_TELEGRAM_ID}: {username}"
            )
    ...
    if existing_admin:
        ...
        if updated:
            ...
            logger.info(
                f"Updated admin {DEFAULT_SUPER_ADMIN_TELEGRAM_ID} to super_admin"
    ...
    logger.info(
        f"Created default super admin with Telegram ID: {DEFAULT_SUPER_ADMIN_TELEGRAM_ID}"
