...
@pytest.mark.asyncio
async def test_can_purchase_level_1_without_partners(session: AsyncSession):
...
@pytest.mark.asyncio
async def test_cannot_skip_levels(session: AsyncSession):
...
@pytest.mark.asyncio
async def test_invalid_level_rejected(session: AsyncSession):
    """Test that invalid levels are rejected."""
    service = DepositValidationService(session)
    can_purchase, error = await service.can_purchase_level(user_id=1, level=99)
    assert can_purchase is False
    assert "Неверный уровень" in error


@pytest.mark.asyncio
async def test_get_available_levels(session: AsyncSession):
    """Test getting available levels for user."""
    service = DepositValidationService(session)
    levels_status = await service.get_available_levels(user_id=1)
